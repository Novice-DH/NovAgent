...
## graph.nodes — 规划/执行/验证节点与路由

模块 `novagent.graph.nodes` 实现 LangGraph 节点函数与条件路由。节点与 LangGraph 节点签名兼容：第一个位置参数为状态 dict，返回普通 dict 更新；`model=None` 时内部 `create_model()`，支持离线注入 FakeModel。

- 模块常量：
  - `PLANNER_PROMPT`：planner 系统提示（英文），说明职责（产出计划）与输出约定（调用 `todo_write` 工具提交计划）；
  - `VERIFIER_PROMPT`：verifier 系统提示（英文），说明职责（只读核查验收标准）与输出约定（最终回复为 JSON `{passed, reason, checks, recommended_next_instruction}`）。
- `planner_node(state, *, model=None) -> dict`：
  1. `todo_write = create_todo_write_tool()`；`agent = model.bind_tools([todo_write])`；
  ...
  6. 无 tool_call 或提取失败：抛出带说明的 `ValueError`。
- `actor_node(state, *, model=None, on_event=None) -> dict`：
  ...
  4. 循环中 `todo_update` tool_call 按 `todo_id` 合并进 todos（更新 `status`/`note`）；
  5. 返回更新：`messages`（仅循环新增的 AIMessage/ToolMessage，不含 System/Human 初始消息）、`last_actor_summary`（最后一轮 AI 文本）、`todos`（发生 todo_update 时）。
- `verifier_node(state, *, model=None) -> dict`：
  ...
  3. 最终 AI 文本解析 JSON `{passed, reason, checks, recommended_next_instruction}`（容忍代码围栏包裹）；解析失败按判定失败处理；
  ...
  6. 返回更新：`passed`、`attempts`（原值 +1）、`verification_results`、`verification_checks`（来自 LLM checks，补齐缺失键为 `""`/`False`）、`last_error`（失败时为 reason 或解析诊断，通过时为 `""`）、`final_answer`（整体通过时为 reason）、`todos`（整体通过 → 非 completed 全标 `completed`；失败 → in_progress 全标 `blocked`）。
- `verifier_route(state) -> str`：`state.get("passed")` 为真 → `"final"`；`state.get("attempts", 0) >= state.get("max_attempts", 3)` → `"final"`；否则 → `"planner"`。

## tools.todo_tools

- `create_todo_write_tool() -> StructuredTool`，名 `todo_write`：args schema 含 `plan_summary: str`、`todos`（`{id, content, status, note}` 数组）、`acceptance_criteria: list[str]`、`verification_commands: list[str]`；无副作用，invoke 返回确认文本（LLM 结构化输出载体）。
...
- `tests/test_graph_state.py` 离线覆盖 LangGraph 共享状态：TypedDict 存在且字段注解逐一正确、`NovGraphState` 的 `total` 为 False、`messages` 注解为 `Annotated[list[BaseMessage], add_messages]` 且导入来源正确、`add_messages` 合并语义（直接调用追加 + 最小 StateGraph 端到端消息拼接不覆盖）、新增 `VerificationCheck` 与 `NovGraphState` 新字段注解；
- `tests/test_graph_nodes.py` 以 FakeModel 离线覆盖节点与路由：planner 首次生成（绑定 `todo_write`、首轮消息、计划提取与 todos 归一）、planner 按失败修订（HumanMessage 含 last_error）、actor ReAct 循环（事件序列、messages 追加、`last_actor_summary`、写文件生效、6 工具绑定）、actor 收集 `todo_update` 回写 todos、verifier 通过（绑定只读工具、命令执行 `VerificationResult`、checks、todos 全 completed、attempts+1、`last_error` 清空）、verifier 失败（非零退出命令、`last_error`、in_progress 标 blocked）、`verifier_route` 三分支与 `max_attempts` 缺省默认 3；
- todo 工具测试覆盖 args schema 字段与无副作用确认文本；
