...
## prompts.stage4 — 上下文压缩提示

模块 `novagent.prompts.stage4` 提供 `CONTEXT_COMPRESSION_PROMPT` 常量（非空英文字符串，逐字使用用户给定原文），为 `context_compressor_node` 的系统提示：
...
- `context_compressor_node(state, *, model=None) -> dict`：上下文压缩节点（压缩机制第二步：压缩执行，`context_monitor_route` 路由目标 `"context_compressor"` 的落地；本 change 中未接入 `build_workflow`）。`model=None` 时内部 `create_model()`；解析或持久化失败均不抛异常。
  ...
  4. JSON 解析：复用 `_parse_verifier_json`（容忍代码围栏与前后杂文本）；解析成功取九键 `summary`/`active_goal`/`completed_work`/`open_todos`/`important_files`/`tool_findings`/`sources`/`next_steps`/`risks`（缺失补 `""`，值一律转 str）；解析失败走确定性回退——`summary` 为既有消息文本的顺序拼接经截断，其余八键为 `""`；
  5. 截断：`summary` 经 `_short_text(…, 1600)`（`CONTEXT_SUMMARY_LIMIT`）；其余八键各经 `_short_text(…, 800)`；截断辅助复用 `novagent.graph.memory`（`_short_text`/`_trim_handoffs`/`_norm_sources` 与既有上限常量）；
  6. token 估算：模块内 `_estimate_text_tokens(text) = max(1, len(text) // 4)`（空文本为 0，不引入新依赖；命名区别于 monitor 的 `_estimate_tokens(messages, model)`）；压缩前 token 数 = 对 transcript 的估算，压缩后 token 数 = 对截断后 summary 的估算；
  7. 返回更新 dict 恰含：
     - `"messages": [RemoveMessage(id=REMOVE_ALL_MESSAGES), AIMessage(content=<截断后 summary>)]`（适配 `add_messages` reducer：先清空全部旧消息再追加摘要消息）；
     - `"context_summary": <截断后 summary>`、`"context_should_compress": False`（刚压缩过）、`"context_token_count": <压缩后估算 token 数>`；
     - `"research_notes": _short_text(state 值, 1600)`、`"agent_handoffs": _trim_handoffs(state 值)`（最近 6 条）、`"sources": _norm_sources(state 值)`（仅 title/url）、`"code_agent_summary": _short_text(state 值, 1000)`；
